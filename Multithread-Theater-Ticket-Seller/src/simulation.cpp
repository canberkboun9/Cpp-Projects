...
//teller lock
pthread_mutex_t tellerLock;
//writing into file lock
...
*/
// I had an idea about making the project an object oriented one. But I had 3 hours, so I did not start to do it
// that way, instead I worked with global variables etc. But that is not the way to do this project I think.
...
int main(int argc, char *argv[]) {
    ...
    for(int i = 0; i< 3; i++){
        tellerFlag.push_back(0); // 0 means available.
    }
...
void *teller(void *param){
    ...
    if(namex==0){
        ...
        name="B";
        usleep(1000*2); //sleep for sync.
    }else if(namex==2){
        name="C";
        usleep(1000*5); //sleep for sync.
    }
    fout << "Teller " << name << " has arrived." <<endl;
    usleep(1000*3); //sleep for 2 ms for sync.
    ...
    while(remainingClients!=0){
        if(namex==1){
            usleep(1000*2); //sleep for sync to avoid selecting the same client
        }else if(namex==2){
            usleep(1000*4); //sleep for sync to avoid selecting the same client
        }

        pthread_mutex_lock(&tellerLock);
        string this_client_name = client_name;
        int this_arrival_time = arrival_time;
        int this_service_time = service_time;
        int this_seat_num = seat_num;
        if(remainingSeats==0){
            seat_arranged = 0;
        }
        else if(seats[seat_num]==-1){
            seat_arranged = this_seat_num;
            seats[this_seat_num] = 0;
            numOfSeats--;
        }
        else{
        ...
        pthread_mutex_unlock(&tellerLock);
        usleep(1000*this_service_time); // sleep until service time

        pthread_mutex_lock(&writeLock); // lock the output file as writing into it is a critical section
        if(seat_arranged==0){
            fout << this_client_name << " requests seat " << this_seat_num << ", reserves None. Signed by Teller " << name << "." << endl;
        }else{
            fout << this_client_name << " requests seat " << this_seat_num << ", reserves seat " << seat_arranged << ". Signed by Teller " << name << "." << endl;
        }
